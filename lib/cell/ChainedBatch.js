...
export default class extends AbstractChainedBatch {
  ...
  _putRaw(key, value, options) {
    let [row, col] = key

    this._body.write(
      `<entry>` +
        `<batch:operation type="update"/>` +
        `<id>https://spreadsheets.google.com/feeds/cells/${this._db.location}/private/full/R${row}C${col}</id>` +
        `<gs:cell row="${row}" col="${col}" inputValue="${value}"/>` +
      `</entry>`
    )
  }

  _put(buf, json, options) {
    this._putRaw(decode(buf), JSON.parse(json), options)
  }

  _del(buf, options) {
    this._putRaw(decode(buf), "", options)
  }
